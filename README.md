...
To prevent pathfinding coordinates from extending beyond the maze boundaries, a helper method named 
`identifyBoundaries()` was implemented. This method iterated through the newly created maze, identifying the minimum 
and maximum x- and y-values where walls were present. Another helper method, `isWithingBoundaries(x, y)`, utilized this 
information to determine whether false-values should be added to coordinates outside the walls.
...
endpoint. This heuristic value was calculated by taking the absolute value, both in X and Y direction. Apart from this 
distinction, the A* algorithm shared similarities with the other algorithms.
...
them to the view to show the user the final result.
...
## Discussion
Throughout this project, the main goal was to create a maze-solving application in Java with a GUI adhering to the MVC
design pattern. The aim was also to implement two variations of Dijkstra's algorithm using different data structures, 
as well as the A* algorithm for pathfinding in the maze. These goals have successfully been achieved as a functional 
maze-solving application with a GUI using the MVC design pattern has been implemented. The application allows the 
user to select a maze image, choose start and end coordinates, and initiate the pathfinding algorithms.

Regarding the implementation of the algorithms, two variations of Dijkstra's algorithm were successfully implemented; 
one utilizing a heap with a PriorityQueue and the other employing a Deque. These algorithms effectively calculated the 
shortest path in the maze. Additionally, the A* algorithm was researched and implemented, which incorporated a heuristic 
value to estimate the distance from the current point to the endpoint. Although the learning materials did not cover 
the A* algorithm, the initiative to study and understand its principles were taken, and it was successfully 
implemented in the application.

While a few challenges were encountered during the image processing phase, these were overcome by conducting additional
research and applying problem-solving techniques. Ultimately, a functioning application that met the initial goals set 
for the project was developed.

### Outcome and result
A few notable differences in the performance of the implemented algorithms could be observed, Figure 2 shows an example
of a standard output. Dijkstra's algorithm with a PriorityQueue was generally the slowest, likely due to the additional 
computational overhead of maintaining element priorities. Dijkstra's algorithm with a Deque showed improved performance,
leveraging a simple FIFO structure for efficient traversal. Generally, the A* algorithm stood out as the fastest, 
utilizing a heuristic value to estimate distances and make informed decisions during pathfinding, going towards the 
goal. However, as explained in the introduction, the A* algorithm is not always the fastest in mazes since the shortest
path may actually lead away from the finish point, Figure 3 shows an example of this. These outcomes underscore the 
importance of algorithmic efficiency and data structure selection. Overall, the A* algorithm proved to be the most 
efficient and effective approach for maze pathfinding in this project.

![](outcome2.png)<br>
Figure 2. Image showing a general outcome of the program.

![](outcome.png)<br>
Figure 3. Image showing an example where the A* algorithm actually is not the fastest.

### Run time complexity
DETTA HÅLLER INTE EMMA MED OM?? Overall time complexity är väl O(rows*columns log(rows*columns)) för man måste kolla på 
solvepath och converttomazepoints???
...
- Converting the maze to MazePoint objects has a time complexity of O(n), where N is the total number of points in the 
  maze. It involves iterating over the maze array and creating MazePoint objects for each point.  //NEJ??
...
In conclusion, we are satisfied with the outcomes of this project as we successfully fulfilled the purpose of creating
a maze-solving application with a GUI, implemented multiple pathfinding algorithms, and achieved the desired 
functionality. This project has provided us with valuable learning experiences, allowing us to enhance our programming 
skills and deepen our understanding of algorithmic problem-solving. The learning modules of the course has provided us 
with background knowledge of different datastructures and algorithms to complete this project.